	...
	for (const change of changes) {
		...
		if ('remove' === change.type) {
			if (!change.selected && selectedWfNode.value?.uuid === change.id) {
				...
				const idx = props.workflow.nodes.findIndex((node: { uuid: string }) => node.uuid === change.id)

				if (idx > -1) {
					props.workflow.nodes.splice(idx, 1)
					wfStore.deleteNode(props.workflow.uuid, change.id)
				}
			}
		}
